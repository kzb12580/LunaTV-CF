...
export async function fetchDoubanCategories(
  ...
  const target = useTencentCDN
    ...
    : useAliCDN
      ? `https://m.douban.ali.cmliussss.com/rexxar/api/v2/subject/recent_hot/${kind}?start=${pageStart}&limit=${pageLimit}&category=${category}&type=${type}`
...
export async function fetchDoubanList(
  ...
  const target = useTencentCDN
    ...
    : useAliCDN
      ? `https://movie.douban.ali.cmliussss.com/j/search_subjects?type=${type}&tag=${tag}&sort=recommend&page_limit=${pageLimit}&page_start=${pageStart}`
...
async function fetchDoubanRecommends(
  ...
  const baseUrl = useTencentCDN
    ...
    : useAliCDN
      ? `https://m.douban.ali.cmliussss.com/rexxar/api/v2/${kind}/recommend`
